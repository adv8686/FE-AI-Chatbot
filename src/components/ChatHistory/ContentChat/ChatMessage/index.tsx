...
import ModalAddFaq from '@components/ChatHistory/ModalAddFaq';
...
const ChatMessage = ({ control }: any) => {
  const refModalAddFaq: any = useRef();
  ...
  return (
    <>
        ...
        <div className='flex flex-col gap-6 max-h-[640px] overflow-auto pt-4 pr-4 scroll-custom'>
          {DataChatSettle?.map((item) => {
            return (
              <div key={item?.id}>
                {item?.user === 'bot' ? (
                  <MessaeBot refModalAddFaq={refModalAddFaq} item={item} />
                ) : (
                  <MessaeUser item={item} />
                )}
              </div>
            );
          })}
      ...
      <ModalAddFaq ref={refModalAddFaq} />
    </>
  );
};
export default ChatMessage;

const MessaeBot = ({ item, refModalAddFaq }: any) => {
  return (
    <div className='flex gap-2 w-6/12'>
      ...
      <div className='flex flex-col gap-2'>
        ...
        {item?.user === 'bot' && (
          <Button
            size='md'
            onClick={() => refModalAddFaq.current.onOpen()}
